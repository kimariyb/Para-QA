...
# MinerU is optional. It is loaded only after argument parsing so ``--help``
# and unrelated Para-QA commands work in lightweight development environments.
_api_client: Any
guess_suffix_by_path: Any
SUPPORTED_INPUT_SUFFIXES: set[str] = set()


def load_mineru() -> None:
    """Load MinerU dependencies immediately before a real parsing run."""
    global _api_client, guess_suffix_by_path, SUPPORTED_INPUT_SUFFIXES
    from mineru.cli import api_client
    from mineru.cli.common import image_suffixes, office_suffixes, pdf_suffixes
    from mineru.utils.guess_suffix_or_lang import guess_suffix_by_path as guess_suffix

    _api_client = api_client
    guess_suffix_by_path = guess_suffix
...
def main() -> None:
    ...
    try:
        ...
        if exc.name == "mineru":
            raise SystemExit("MinerU is required for parsing. Install it from requirements.txt.")
        raise
